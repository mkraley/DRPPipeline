...
| Module | Purpose |
|--------|--------|
| **sourcing** | Sources candidate URLs into the DB (spreadsheet or ADC Figshare API, selected by `source` in config). |
| **collector** | Collects sourced projects into local folders (Socrata, catalog.data.gov, CMS, USFS, ADC, BTS, etc.; selected by `source` in config). |
...
- **Backend:** `interactive_collector/api.py` — Blueprint with `/api/projects/*`, `POST /api/projects/add` (new project from a source URL; then load + same flow as Copy & Open), `/api/projects/load`, `/api/scoreboard`, `/api/save`, `/api/download-file`, `POST /api/download-batch/preview|start|control` (batch data-file downloads with rate limiting and pause/resume), `/api/pipeline/*`, `/api/proxy`, `/api/extension/save-pdf`, `/api/metadata-from-page`, `/api/downloads-watcher/*`, plus `/api/chat/query` and `/api/chat/confirm` for pipeline chat.
