...
| **collector** (NPS) | When `source` is `nps`, downloads public IRMA Digital Files for each sourced Project. Products become subfolders named from the product title (no IRMA id). Data Table Info is saved as UTF-8-sig per-file CSVs (`{stem}_data_table_info.csv`) in `_project_files` and each product folder. Landing-page metadata is written as `project_metadata.json` and `product_metadata.json`, including the IRMA Collection > Program > Project breadcrumb (products append a Product segment), Units/Geography, and any DOI from the citation. After collection, `projects.num_files` and `nps_projects.public_file_count` are set to the recursive on-disk file count (product folders plus generated JSON/CSV). Agency is Department of the Interior; office is National Park Service. External Links and restricted files are skipped. Processes `status="sourced"`. |
