...
| **collector** (NPS) | When `source` is `nps`, downloads public IRMA Digital Files for each sourced Project. Products become subfolders. Data Table Info is saved as UTF-8-sig CSV (`data_table_info.csv` plus per-file sidecars) in `_project_files` and each product folder. Landing-page metadata is written as `project_metadata.json` and `product_metadata.json`, including Units/Geography. Agency is Department of the Interior; office is National Park Service. External Links and restricted files are skipped. Processes `status="sourced"`. |
