...
class TestApiProjectsLoad(unittest.TestCase):
    ...
    def test_projects_load_delete_folder_on_load_false(self) -> None:
        """POST /api/projects/load can preserve the output folder."""
        ...
        with patch("interactive_collector.api.get_project_by_drpid", return_value=proj):
            with patch("interactive_collector.api.ensure_output_folder") as mock_ensure:
                ...
                resp = self.client.post(
                    "/api/projects/load",
                    json={"drpid": 1, "delete_folder_on_load": False},
                    content_type="application/json",
                )
        self.assertEqual(resp.status_code, 200)
