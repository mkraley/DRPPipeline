...
class GoogleSheetUpdater:
    ...
    def _find_row_by_url(
        ...
        """
        Return 1-based row number where URL column exactly matches source_url, or None.

        Only exact string match (after strip + lower) counts. Catalog URL variants
        (e.g. ``RDS-2022-0015`` vs ``RDS-2022-0015-4``) are distinct; no match
        causes the caller to append a new row.
        """
